...
import { nodeFromHtml } from './storyHelpers.js';
...
function renderModal({ onClose, onOpen, openOnLoad, title }) {
  ...
  activeController = modal(overlay, {
    focusTarget: () => overlay.querySelector('input'),
    onClose,
    onOpen,
  });
  trigger.addEventListener('click', (event) => activeController.open({ reason: 'trigger', event }));
...
function renderFrame({ title }) {
  ...
  frame.querySelectorAll('[data-modal-close]').forEach((button) => { button.disabled = true; });
